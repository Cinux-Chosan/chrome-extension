...
// 页面过滤、搜索、展示等相关逻辑
const { searchText, filteredCookies, domains, copyCookie, copyCookies } = (() => {
  ...
  const copyCookie = async (key: string, index: number, type = "text") => {
    ...
    ElMessage.success('导出成功，已复制到剪切板！')
  }

  const copyCookies = async (domain?: string, type = "text") => {
    const cookies = domain ? filteredCookies.value[domain] : Object.values(filteredCookies.value).flat()
    const str = type === 'text' ?
...
<template>
  <div class="cookie-nav">
    ...
    <el-dropdown size="small" class="cookie-item-header-btn" :teleported="false">
      <el-link @click.stop="copyCookies(undefined, 'text')">
      ...
      <template #dropdown>
        <el-dropdown-menu>
          <el-dropdown-item @click.stop="copyCookies(undefined, 'json')">
            导出 JSON</el-dropdown-item>
        </el-dropdown-menu>
      </template>
    </el-dropdown>
    <el-link @click="toggleSubscribeDrawer">我的订阅</el-link>
  </div>
          ...
          <template #title>
            <div class="collapse-header-content">
              <span class="domain">{{ domain }}</span> ({{
                  filteredCookies[domain].length
              }})
              <el-button type="primary" size="small" class="cookie-item-header-btn" @click.stop="applyToCurrent(domain)"
                v-if="isHttpTab">应用</el-button>
              <el-button type="primary" size="small" class="cookie-item-header-btn" @click.stop="bindToCurrent(domain)"
                v-if="isHttpTab">订阅</el-button>
              <el-dropdown size="small" class="cookie-item-header-btn" :teleported="false">
                <el-button type="primary" size="small" @click.stop="copyCookies(domain, 'text')">
                  导出<el-icon class="el-icon--right">
                    <arrow-down />
                  </el-icon>
                </el-button>
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item @click.stop="copyCookies(domain, 'json')">
                      导出 JSON</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
              <el-button type="primary" size="small" class="cookie-item-header-btn" @click.stop="delAllCookie(domain)">
                删除</el-button>
            </div>
          </template>
              ...
              <el-table :data="filteredCookies[domain]" stripe style="width: 100%">
                <el-table-column prop="name" label="名称" fixed min-width="140" show-overflow-tooltip>
                  <template #default="{ row, $index }">
                    <ContentEditable :value="row.name" class="content-editable" @blur="
                      (evt) =>
                        updateField(
                          domain,
                          $index,
                ...
                <el-table-column prop="value" label="值" min-width="140" show-overflow-tooltip>
                  <template #default="{ row, $index }">
                    <ContentEditable :value="row.value" class="content-editable" @blur="
                      (evt) =>
                        updateField(
                          domain,
                          $index,
                ...
                <el-table-column prop="path" label="路径" show-overflow-tooltip>
                  <template #default="{ row, $index }">
                    <ContentEditable :value="row.path" class="content-editable" @blur="
                      (evt) =>
                        updateField(
                          domain,
                          $index,
                ...
                <el-table-column prop="expirationDate" label="过期时间" min-width="250">
                  <template #default="{ row, $index }">
                    <el-date-picker :model-value="row.expirationDate * 1000 || null" @update:modelValue="
                      async (val: number) => {
                        if (val && val <= Date.now()) {
                          await ElMessageBox.confirm(
                            `选择时间小于等于当前时间， cookie 将被自动清除`
                          );
                        }
                        updateField(
                          domain,
                ...
                <el-table-column prop="sameSite" label="Same Site" min-width="140" show-overflow-tooltip>
                  <template #default="{ row, $index }">
                    <el-select :model-value="row.sameSite" @change="
                      async (val: SameSiteStatus) => {
                ...
                <el-table-column prop="secure" label="Secure">
                  <template #default="{ row, $index }">
                    <el-switch :model-value="row.secure" inline-prompt active-text="Y" inactive-text="N" @change="
                      async (val: boolean) => {
                ...
                <el-table-column fixed="right" width="140">
                  <template #header>
                    <div class="flex-op">
                      操作
                      <el-button type="primary" size="small" link @click="onEdit(domain)">
                        <el-icon>
                          <Plus />
                        </el-icon>
                      </el-button>
                  ...
                  <template #default="{ $index }">
                    <div class="table-op">
                      ...
                      <el-dropdown size="small">
                        <el-button link type="primary" size="small" @click="copyCookie(domain, $index, 'text')">
                          导出<el-icon class="el-icon--right">
                            <arrow-down />
                          </el-icon>
                        </el-button>
